const {join, dirname, relative, basename, normalize} = require('path');
...
function normalizeSource(source, context) {
  ...
  const relativeSource =
    sourceRoot === packageRoot
      ? relative(packageRoot, source).replace(/\\/g, '/')
      : relative(packageRoot, sourceRoot).replace(/\\/g, '/');

  const sourceBasename = basename(relativeSource);
  const sourceDir = IS_FILE.test(relativeSource)
    ? dirname(relativeSource).replace(/\\/g, '/')
    : relativeSource;
